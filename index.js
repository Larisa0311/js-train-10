  ...
  if (arguments.length > 1) {
    const objAssign = Object.assign(...objects);
     for (const key in objAssign) {
      let value = objAssign[key];

         if (typeof value === "string") {
         objAssign[key] = value;
       } else { objAssign[key] = value.toString() };
      };
...
function customObjectFromEntries(entries) {
  ...
  if (Array.isArray(entries) === true) {
    const entriesMap = entries.map(([key, value]) => { 
      return ((typeof key) !== "string") ? [key.toString(), key.toString()] : [key, value];
    });
...
function keysAndValues(obj) {
  ...
  if (Object.keys(obj).length > 0) {
    ...
    return Array.of(arrKeys, arrValue);
...
function filterObjectsByKey(arr, key) {
  ...
  if (Array.isArray(arr) === true) {
    const objectByKey = arr.filter((element) => element.hasOwnProperty(key) === true);
...
function checkOwnership(obj1, obj2, key) {
  ...
  const keyObj1 = obj1.hasOwnProperty(key);
  const keyObj2 = obj2.hasOwnProperty(key);

  if (keyObj1 === true && keyObj2 === true) {
    return true;
  } else {return false};
}
...
function getObjectValuesSum(obj) {
  ...
  if (Object.keys(obj).length > 0) {
    ...
    const sumValue = value.reduce((sum, elem) => sum + elem, 0);
...
function convertArrayToObj(arr) {
  ...
  if (Array.isArray(arr) === true) {
    ...
    //   let [key, value] = arr[i];
    //   objToConvert[key] = value; 
    // };
       
    // return objToConvert;

    
    let objConv = Object.fromEntries(arr);
    return objConv;
